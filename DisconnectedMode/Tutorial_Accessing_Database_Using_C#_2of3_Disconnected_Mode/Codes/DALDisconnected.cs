...
namespace DisconnectedModeExample
{
    public class DALDisconnected
    {
        ...
        // in the example below, the whole database with three tables are selected.
        ...
        public void StudentCreate(Student student)
        {
            ...
            var cmd = new SQLiteCommandBuilder(DataAdapter);
            // update the database
        ...
        public List<Enrollment> EnrollmentReadAll()
        {
            ...
            {
                var id = Int32.Parse((row["EnrollmentID"].ToString()));
                var studentId = row["StudentID_FK"].ToString();
                var subjectId = row["SubjectID_FK"].ToString();
                enrollments.Add(new Enrollment(id, studentId, subjectId));
            }
            return enrollments;
        }
    }
